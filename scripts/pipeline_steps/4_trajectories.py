...
output_dir = PROCESSED_DATA_DIR / f"{today}_patient_trajectories_{cancer_type}"
...
metadata_path = str(INTERIM_DATA_DIR / "20240930_preprocessed_KIRC" / "CuratedClinicalData.csv")
...
logger.info(f"Metadata: {metadata_selection.shape[0]} patients × {metadata_selection.shape[1]} features")
...
if X_train_path.exists() and X_test_path.exists():
    ...
    logger.info(f"Created {random_connections_train.shape[0]} connections in train data")
...
logger.info(f"Saved: nodes_metadata.csv")

# Save all distances
all_traj.to_csv(output_dir / 'all_distances.csv', index=False)
logger.info(f"Saved: all_distances.csv")
...
logger.info(f"Saved: trajectories.csv")

# Save trajectory mapping
trajectory_mapping_df.to_csv(output_dir / 'trajectory_mapping.csv', index=False)
logger.info(f"Saved: trajectory_mapping.csv")

# Save train/test splits if generated
if use_train_test_split:
    random_connections_train.to_csv(
        output_dir / f'random_connections_to_{link_next}_next_train.csv',
        index=False
    )
    random_connections_test.to_csv(
        output_dir / f'random_connections_to_{link_next}_next_test.csv',
        index=False
    )
...
logger.info(f"Created {len(random_connections_negatives)} control connections (self-connections removed)")
...
if use_train_test_split:
    ...
    controls_train = controls_train[controls_train["source"] != controls_train["target"]].reset_index(drop=True)
    ...
    controls_test = controls_test[controls_test["source"] != controls_test["target"]].reset_index(drop=True)
...
model_dir = Path("models/20251212_models_KIRC")
...
recnet_model.load_state_dict(torch.load(recnet_model_path, map_location=device, weights_only=False))
recnet_model.eval()
logger.info(f"Reconstruction network loaded from: {recnet_model_path}")

# ============================================================================
# CRITICAL: Create scaler fitted on TRAIN split only (avoid data leakage)
# ============================================================================
logger.info("-" * 80)
...
X_train = pd.read_csv(X_train_path, index_col=0)
logger.info(f"Loaded train split: {X_train.shape[0]} patients × {X_train.shape[1]} genes")

# Fit scaler ONLY on train data to avoid data leakage
# X_train is (patients × genes) which is what scaler expects
scaler = MinMaxScaler()
scaler.fit(X_train.values)
logger.info(f"Scaler fitted on TRAIN split: {X_train.shape[0]} patients × {X_train.shape[1]} genes")
...
if use_train_test_split:
    ...
    train_synthetic_dir = PROCESSED_DATA_DIR / f"{today}_synthetic_data" / cancer_type.lower() / "recnet" / "early_to_late" / "train_to_train"
    ...
    for traj_idx, trajectory in enumerate(train_trajectories):
        if (traj_idx + 1) % 50 == 0 or traj_idx == 0:
            logger.info(f"Train progress: {traj_idx + 1}/{len(train_trajectories)} trajectories generated")
    ...
    test_synthetic_dir = PROCESSED_DATA_DIR / f"{today}_synthetic_data" / cancer_type.lower() / "recnet" / "early_to_late" / "test_to_test"
    test_synthetic_dir.mkdir(parents=True, exist_ok=True)

    for traj_idx, trajectory in enumerate(test_trajectories):
        if (traj_idx + 1) % 50 == 0 or traj_idx == 0:
            logger.info(f"Test progress: {traj_idx + 1}/{len(test_trajectories)} trajectories generated")
    ...
    synthetic_output_dir = PROCESSED_DATA_DIR / f"{today}_synthetic_data" / cancer_type.lower() / "recnet" / "early_to_late"
    synthetic_output_dir.mkdir(parents=True, exist_ok=True)

    logger.info(f"Generating {len(trajectories)} trajectories with {n_timepoints} timepoints each")

    for traj_idx, trajectory in enumerate(trajectories):
        if (traj_idx + 1) % 50 == 0 or traj_idx == 0:
            logger.info(f"Progress: {traj_idx + 1}/{len(trajectories)} trajectories generated")
...
logger.info(f"  - trajectory_mapping.csv: {len(trajectory_mapping_df)} patient-trajectory mappings")
logger.info(f"  - all_negatives.csv: {len(random_connections_negatives)} control connections")
if use_train_test_split:
    logger.info(f"  - Train/test splits saved")
    logger.info(f"  - Synthetic data (train): {len(train_trajectories)} trajectory files in train_to_train/")
    logger.info(f"  - Synthetic data (test): {len(test_trajectories)} trajectory files in test_to_test/")
else:
