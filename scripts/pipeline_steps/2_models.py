...
from renalprog.config import RAW_DATA_DIR, INTERIM_DATA_DIR, get_dated_dir,VAEConfig, get_dated_dir, MODELS_DIR
...
# path_rnaseq = preprocessed_data_path / "rnaseq_preprocessed.csv"
...
test_size = 0.2
logging.info(f"Splitting into train and test set with {int((1-test_size)*100)}/{int(test_size*100)} ratio")
...
from renalprog.modeling.train import train_vae, train_vae_with_postprocessing
...
vae_config.EPOCHS = 200* vae_config.BETA_CYCLES  # Total epochs = epochs per cycle * number of cycles
...
logging.info(f"VAE training history plot saved to {model_dir / 'vae' / 'vae_training_history'}")
...
logging.info(f"Reconstruction network history plot saved to {model_dir / 'reconstruction_network_history'}")
