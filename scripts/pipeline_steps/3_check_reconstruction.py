...
from renalprog.config import (
    ...
    MODELS_DIR, REPORTS_DIR, FIGURES_DIR, VAEConfig, get_dated_dir
)
...
path_rnaseq = "data/interim/20240930_preprocessed_KIRC/rnaseq_maha.csv"
path_clinical = "data/interim/20240930_preprocessed_KIRC/CuratedClinicalData.csv"

preprocessed = load_rnaseq_data(path_rnaseq).T
...
umap_path = os.path.join(FIGURES_DIR, f'{date_umap}_umap_reconstruction')
...
logging.info(f"Preprocessed data UMAP saved to: {os.path.join(umap_path, 'preprocessed')}")
...
model_dir = "models/20251212_models_KIRC"
...
reconstruction_x, _, _, z, scaler = apply_VAE(
    torch.tensor(preprocessed.values).to(torch.float32),
    model_vae,
    y=None
)
...
model_net.load_state_dict(torch.load(network_model_path, map_location=device, weights_only=False))
model_net.eval()
...
logging.info(f"Comparison UMAP (original vs VAE) saved to: {os.path.join(umap_path, 'preprocessed_and_vae')}")
...
logging.info(f"Comparison UMAP (original vs RecNet) saved to: {os.path.join(umap_path, 'preprocessed_and_recnet')}")
...
path_synth_data_evaluation = os.path.join(PROCESSED_DATA_DIR, f"{date_umap}_reconstruction_evaluation")
path_synth_plots_evaluation = os.path.join(FIGURES_DIR, f"{date_umap}_reconstruction_evaluation")
...
reconstruction_test, _, _, _, _ = apply_VAE(
    torch.tensor(X_test.values).to(torch.float32),
    model_vae,
    y=None
)
